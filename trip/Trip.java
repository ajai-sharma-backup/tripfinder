...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        assert (from == segment.get(0));
        while (!segment.isEmpty()) {
            reportSegmentHelper(seq, segment);
            seq += 1;
        }
        return seq;
    }

    /** Helper function for reportSegment.
     *  Reports one road length with number SEQ.
     *  Deletes items from SEGMENT. */
    void reportSegmentHelper(int seq, List<Integer> segment) {
        Road currentRoad = _map.getLabel(segment.get(0), segment.get(1));
        Road firstRoad = currentRoad;
        double d = 0.0;
        while (segment.size() > 1 && currentRoad.same(firstRoad)) {
            currentRoad = _map.getLabel(segment.get(0), segment.get(1));
            if (!currentRoad.same(firstRoad)) {
                break;
            }
            d += currentRoad.length();
            segment.remove(0);
        }
        if (segment.size() == 1) {
            System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                seq, firstRoad, firstRoad.direction().fullName(), d,
                _map.getLabel(segment.get(0)));
            segment.remove(0);
        } else {
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            return _map.getLabel(u).dist(_map.getLabel(v));
        }
