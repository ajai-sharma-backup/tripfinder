...
class Road {
    ...
    /** Returns true if ROAD has same name and direction. */
    boolean same(Road road) {
        return direction().equals(road.direction())
                && toString().equals(road.toString());
    }

    /** Displays the road; for debugging. */
    void displayRoad() {
        System.out.println(toString() + direction().fullName() + length());
    }
